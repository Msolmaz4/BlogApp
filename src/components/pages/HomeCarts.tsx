...
const HomeCarts = () => {
  ...
  <div className="flex-none w-52 relative">
    <img src={item.image} alt="" className="absolute inset-0 w-full h-full object-cover rounded-lg" loading="lazy" />
                ...
                <div className="ml-2"> 
                    <h3 className="text-slate-900 font-semibold text-center">{item.title}</h3>
                <p className="text-slate-700 text-justify">{item.content.split(" ").slice(0, 10).join(" ")}....</p>
                <p className="text-slate-400 text-justify"> Published Date:{item.createdAt.slice(0, 10)}</p>
                <div className="mt-6 gap-2 justify-between flex">
                <div className="flex gap-4 ">
                {kalp ?<FaHeart className="" size={24} /> : <CiHeart size={24}/> }
                
               <FaEye size={24}/> 
                 <MdOutlineComment size={24} /></div>
                <div>
